...
int ign_evt = 0; // counts ignition pulses
int last_ign_evt = 0;
int last_last_ign_evt = 0;
int last_last_last_ign_evt = 0;
#define fps 10000
void setup()
{
...
//rpm = 1000000 / tau * 60 / 2; //convert microseconds to rps to rpm (2 ignition events per revolution) 
  ...
  // grab pulses
  interrupts();
  delayMicroseconds(fps);
  noInterrupts();
  
  //calculate RPM
  int all_evt = 0;
  all_evt = (last_last_last_ign_evt + last_last_ign_evt + last_ign_evt + ign_evt)/4;
  rpm = all_evt * (1000000 / fps) * 60 / 2; // convert counts per frame to RPM (2 pulses per revolution)
  last_last_last_ign_evt = last_last_ign_evt;
  last_last_ign_evt = last_ign_evt;
  last_ign_evt = ign_evt;
  ign_evt = 0;
  //Display LED
  displayLED(rpm);
  //Display VFD
